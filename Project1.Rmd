...
ggplot(plot_data, aes(x = Dim1, y = Dim2)) +
  geom_point(aes(color = Author, shape = IsAuthor9), size = 7) +
  scale_shape_manual(
...
```{r}
# making confusion matrices for point estimates obtained above
 
all_classes <- sort(unique(c(DApreds, KNNpreds, testlabels)))

# Convert to factors with consistent levels
DApreds_factor <- factor(DApreds, levels = all_classes)
testlabels_factor <- factor(testlabels, levels = all_classes)
KNNpreds_factor <- factor(KNNpreds, levels = all_classes)


DApointconfusion <- confusionMatrix(DApreds_factor,
                                    testlabels_factor)

KNNpointconfusion <- confusionMatrix(KNNpreds_factor,
                                     testlabels_factor)
DApointconfusion
print("---------------------------------")
KNNpointconfusion
...
```
```{r}
# trying new method for making confusion matrices for LOOCV

all_classes <- sort(unique(c(DApredictions, KNNpredictions, truth)))

# Convert to factors with consistent levels
DApredictions_factor <- factor(DApredictions, levels = all_classes)
truth_factor <- factor(truth, levels = all_classes)
KNNpredictions_factor <- factor(KNNpredictions, levels = all_classes)


DAconfusion <- confusionMatrix(DApredictions_factor,
                                    truth_factor)

KNNconfusion <- confusionMatrix(KNNpredictions_factor,
                                     truth_factor)
print("DA LOOCV matrix for new method:")
DAconfusion
print("DA LOOCV matrix for old method (has warning")
DAconfusionmatrix
print("KNN LOOCV matrix for new method")
KNNconfusion
print("KNN LOOCV matrix for old method")
KNNconfusionmatrix

```
```{r}
# making more tables than ikea
library(knitr)
...
Empirically, KNN makes only one mistake but DA makes 5 mistakes, enough to really justify using KNN more seriously.  

It does have to be noted-however- that both of them mistake one of CBB's books, with DA allocating it to Mary SHelley and KNN allocating it to William Godwin (don't know if its necessarily the same book).
